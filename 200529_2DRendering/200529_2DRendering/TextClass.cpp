...
bool TextClass::SetFps(int fps, ID3D11DeviceContext* deviceContext)
{
	...
	result = UpdateSentence(m_sentence3, fpsString, 20, 20, red, green, blue, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetCpu(int cpu, ID3D11DeviceContext* deviceContext)
{
	...
	strcat_s(cpuString, "%");
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence4, cpuString, 20, 40, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
