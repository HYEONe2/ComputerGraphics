	...
	screenWidth, int screenHeight, D3DXMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, "../Engine/data/Font.txt", L"../Engine/data/Font.dds");
	if (!result)
	{
	...
	// Initialize the first sentence.
	result = InitializeSentence(&m_sentence3, 20, device);
	if (!result)
	{
		return false;
	}
	result = InitializeSentence(&m_sentence4, 20, device);
	if (!result)
	{
		return false;
	}
	result = InitializeSentence(&m_sentence5, 20, device);
	if (!result)
	{
		return false;
	}
	result = InitializeSentence(&m_sentence6, 20, device);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetAmbient(bool Ambient, ID3D11DeviceContext *deviceContext)
{
	char tempString[16];
	char boolString[16];
	bool result;
	...
	if(Ambient)
		strcpy_s(tempString, "True");
	else
		strcpy_s(tempString, "False");
	strcpy_s(boolString, "Ambient: ");
	strcat_s(boolString, tempString);

	result = UpdateSentence(m_sentence4, boolString, 20, 100, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetDiffuse(bool Diffuse, ID3D11DeviceContext *deviceContext)
{
	char tempString[16];
	char boolString[16];
	bool result;
	...
	if (Diffuse)
		...
		strcpy_s(tempString, "False");
	strcpy_s(boolString, "Diffuse: ");
	strcat_s(boolString, tempString);
	
	result = UpdateSentence(m_sentence5, boolString, 20, 120, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetSpecular(bool Specular, ID3D11DeviceContext *deviceContext)
{
	char tempString[16];
	char boolString[16];
	bool result;
	...
	if (Specular)
		...
		strcpy_s(tempString, "False");
	strcpy_s(boolString, "Specular: ");
	strcat_s(boolString, tempString);

	result = UpdateSentence(m_sentence6, boolString, 20, 140, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
