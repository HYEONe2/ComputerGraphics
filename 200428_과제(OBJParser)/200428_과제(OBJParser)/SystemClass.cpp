...
void SystemClass::KeyInput_ForFilter()
{
	if (m_Input->IsKeyDown('1'))
	{
		m_Graphics->Change_Filter(D3D11_FILTER_MIN_MAG_MIP_POINT);
	}
	if (m_Input->IsKeyDown('2'))
	{
		m_Graphics->Change_Filter(D3D11_FILTER_MIN_MAG_MIP_LINEAR);
	}
	if (m_Input->IsKeyDown('3'))
	{
		m_Graphics->Change_Filter(D3D11_FILTER_ANISOTROPIC);
	}
}
