...
void D3DClass::CreateTextFileForMyComputer()
{
	char* chVideoCardName;
	unsigned long bufferSize, i;
	ofstream fout;
	// Get a pointer to the error message text buffer.
	chVideoCardName = m_videoCardDescription;
	// Get the length of the message.
	bufferSize = sizeof(m_videoCardDescription);
	// Open a file to write the error message to.
	fout.open("VideoInfo.txt");
	// Write out the error message.
	fout << "Video Card Name : ";
	for (i = 0; i < bufferSize; i++)
	{
		fout << chVideoCardName[i];
	}
	fout << endl;
	fout << "Memory Amount : " << m_videoCardMemory << endl;
	// Close the file.
	fout.close();
