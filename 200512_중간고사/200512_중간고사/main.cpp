...
bool GraphicsClass::InitializeD3D(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	D3DXMatrixPerspectiveFovLH(&m_projectionMatrix, fieldOfView, screenAspect, 1000.f, 0.1f);

	// Initialize the world matrix to the identity matrix.
	D3DXMatrixIdentity(&m_worldMatrix);

	// Create an orthographic projection matrix for 2D rendering.
	D3DXMatrixOrthoLH(&m_viewMatrix, (float)screenWidth, (float)screenHeight, 1000.f, 0.1f);
	return true;
}
...
bool GraphicsClass::InitializeBuffers()
{
	...
	vertices[0].color = D3DXVECTOR4(0, 1.0f, 1.0f, 1.f);
	vertices[1].color = D3DXVECTOR4(1.0f, 0, 1.0f,1.f);
	vertices[2].color = D3DXVECTOR4(1.0f, 1.0f, 0, 1.f);
	vertices[3].color = D3DXVECTOR4(1.0f, 0, 1.0f, 1.f);
	vertices[4].color = D3DXVECTOR4(0, 1.0f, 1.0f, 1.f);
	vertices[5].color = D3DXVECTOR4(1.0f, 0, 1.0f, 1.f);
	vertices[6].color = D3DXVECTOR4(1.0f, 1.0f, 0, 1.f);
	vertices[7].color = D3DXVECTOR4(1.0f, 0, 1.0f, 1.f);
...
bool GraphicsClass::Frame()
{
	...
	D3DXMatrixLookAtLH(&viewMatrix, &g_position, &lookAt, &up);
	...
	g_iDegree += 1.f;
	D3DXMatrixRotationX(&matRotX, D3DXToRadian(g_iDegree));
	D3DXMatrixRotationY(&matRotY, D3DXToRadian(g_iDegree));
	worldMatrix = worldMatrix * matRotX *matRotY;
	...
	RenderShader(worldMatrix, viewMatrix, projectionMatrix);
		...
		while (!done)
		{
			...
			else
			{
				...
				if (g_keys['S'])
				{
					...
					// Create the rasterizer state from the description we just filled out.
					pGraphics->GetDevice()->CreateRasterizerState(&rasterDesc, &pRasterState);

					pRasterState->GetDesc(&rasterDesc);

					pGraphics->GetDevice()->CreateRasterizerState(&rasterDesc, &pRasterState);
					pGraphics->GetDeviceContext()->RSSetState(pRasterState);
				}
				if (g_keys['W'])
				{
					ID3D11RasterizerState* pRasterState;
